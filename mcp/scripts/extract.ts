...
import type { A11yResults } from "./audit-axe.js";
...
async function main() {
  ...
  // Load axe audit results if present (generated by npm run mcp:audit-a11y)
  const a11yResultsPath = join(MCP_ROOT, "generated", "a11y-results.json");
  let axeResults: A11yResults["components"] = {};
  if (existsSync(a11yResultsPath)) {
    try {
      const raw = JSON.parse(readFileSync(a11yResultsPath, "utf8")) as A11yResults;
      axeResults = raw.components ?? {};
      console.log(`Loaded a11y results for ${Object.keys(axeResults).length} components.`);
    } catch {
      console.warn("Failed to parse a11y-results.json — continuing without axe data.");
    }
  } else {
    console.log("No a11y-results.json found — run npm run mcp:audit-a11y to generate.");
  }
  ...
  for (const name of COMPONENTS) {
    ...
    const axeReport = axeResults[name];
